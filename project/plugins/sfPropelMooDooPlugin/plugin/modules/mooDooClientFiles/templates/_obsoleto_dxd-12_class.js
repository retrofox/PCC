...
mooWin.vtnEdit = new Class({
  ...
  renderWinContent: function(){
    ...
    this.meta = JSON.decode(this.node.getElement('moo_meta').get('text').trim())
    
    // Vamos a guardas las propiedades de la ventana.
    ...
    $btnsActions.each (function ($btn, $iB) {
      ...
      $btnActionsLi.each (function ($btnLi, $iL) {
        ...
        if ($btnLi.get('link_delete')) {
          ...
          // Buscamos el formulario padre para el sf_method
          var $sfAdminListTdActionsForm = $(this.meta.win.sf_form_method);
          ...
          $btnLi.addEvent ('click', function (e) {
            ...
            if (confirm($btnLi.getProperty('msg'))) {
              ...
              var $myHTMLRequest = new Request.HTML({
                evalScripts: false,
                url: $btnLi.getProperty('link_delete'),
                onComplete: function (responseTree, responseElements, responseHTML, responseJavaScript) {
                  var $JSON_response = JSON.decode(responseElements[0].get('text').trim());

                  $JSON_response.actions.each (function ($action, $iA) {
                    switch ($action.type) {
                      case 'win_refresh':
                        this.refresh();
                        break
                      ...
                      case 'delete_row':
                        // Solo borramos el tr del admin
                        $btnLi.getParent('tr').dispose();
                        break;
                    }
                  }, this);
                }.bind (this)
              }).post($sfAdminListTdActionsForm);
  ...
  addSelectWithAdd: function () {
    ...
    $content2Select.each (function ($selWAdd, $iS) {
      ...
      $btns2Select[2].addEvent ('click', function (e) {
        e.stop();
        addWAjax.options.url = $input2add.get('link_to_add');
        addWAjax.send('value='+$input2add.get('value'));
      }.bind(this));
