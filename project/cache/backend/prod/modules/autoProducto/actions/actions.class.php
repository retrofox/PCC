...
class autoProductoActions extends sfActions
{
  ...
  public function executeIndex(sfWebRequest $request)
  {
    ...
    if ($request->hasParameter('only_list')) {
      if ($request->getParameter('only_list')) {
        return $this->renderPartial('producto/win_list_content', array ('pager' => $this->pager, 'sort' => $this->sort, 'helper' => $this->helper, 'only_list' => true));
      }
    }
  ...
  public function executeFilter(sfWebRequest $request)
  {
    if ($request->hasParameter('_reset'))
    {
      ...
      if ($this->getRequest()->isXmlHttpRequest()) {
        $this->redirect('@producto?only_list=true');
      }
      else {
  ...
  public function executeEdit(sfWebRequest $request)
  {
    ...
    $this->winEdit = array (
      'win' => 'edit_win-producto-'.$this->producto->getId()
    );
...
public function executeDelete(sfWebRequest $request)
{
  ...
  if ($this->getRequest()->isXmlHttpRequest()) {
    return $this->renderPartial('producto/data_json-delete');
  }
  else $this->redirect('@producto');
}
  ...
  public function executeBatch(sfWebRequest $request)
  {
    ...
    $validator = new sfValidatorPropelChoiceMany(array('model' => 'Producto'));
    try
    {
      // validate ids
  ...
  protected function executeBatchDelete(sfWebRequest $request)
  {
    ...
    $criteria->add('producto.ID', $ids, Criteria::IN);

    $count = ProductoPeer::doDelete($criteria);
  ...
  protected function processForm(sfWebRequest $request, sfForm $form)
  {
    ...
    if ($form->isValid())
    {
      $this->getUser()->setFlash('notice-producto-edit', $form->getObject()->isNew() ? 'The item was created successfully.' : 'The item was updated successfully.');
      $producto = $form->save();
      ...
      if ($request->hasParameter('_save_and_add'))
      {
        ...
        if ($this->getRequest()->isXmlHttpRequest())
        {
          $this->redirect('producto/newWin');
        }
          else $this->redirect('@producto_new');
        }
      else
      {
        // Modificamos comportamiento si es AJAX
        if ($this->getRequest()->isXmlHttpRequest())
        {
          ...
          $this->redirect('@producto_edit?id='.$producto->getId().'&isCommingEdit=true');
        }
        else $this->redirect('@producto_edit?id='.$producto->getId());
      }
    }
    else
    {
      $this->getUser()->setFlash('error-producto-edit', 'The item has not been saved due to some errors.');
    }
  }
  ...
  protected function addSortCriteria($criteria)
  {
    ...
    $column = ProductoPeer::translateFieldName(sfInflector::camelize(strtolower($sort[0])), BasePeer::TYPE_PHPNAME, BasePeer::TYPE_COLNAME);
    if ('asc' == $sort[1])
    {
  ...
  {
    if (!is_null($sort = $this->getUser()->getAttribute('producto.sort', null, 'admin_module')))
    {
      return $sort;
    }
