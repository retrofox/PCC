 ...
 */
class BaseProducto_categoriaGeneratorHelper extends sfModelGeneratorHelper
{
...
/*
  ...
  {
    return 'list' == $action ? 'producto_categoria' : 'producto_categoria_'.$action;
  }
*/

  public function getUrlForAction($action, $module = '')
  {
    $module_action = $module ? $module : 'producto_categoria';
  ...
  public function mooJsonDataToEditObject($object, $params)
  {
    ...
    if (isset($params['ajax_request'])) {
      ...
      return "{type: 'ajax_link', link: '".url_for($this->getUrlForAction('edit'), $object)."', link_content: '".url_for('producto_categoria/editWinContent?id='.$producto_categoria->getId())."', update: '_new', execute: 'renderAjaxEditWin'},\n";
    }
    else {
  ...
  public function mooJsonDataToListObjects($object, $params)
  {
    ...
    if (isset($params['ajax_request'])) {
      ...
      return "{type: 'ajax_link', link: '".url_for('producto_categoria')."', link_content: '".url_for('producto_categoria/editWinContent?id='.$producto_categoria->getId())."', update: '_new', execute: 'renderAjaxListWin'},\n";
    }
    else {
        return "{type: 'Guarda ACA !!'}";
    };
  ...
  public function mooJsonDataToNew($params, $cssClass='btn_admin_actions')
  {
    if (isset($params['inWinPopUp'])) {
      return "{type: 'ajax_link', link: '".url_for($this->getUrlForAction('new'))."', update: '_new', node_insert: 'embedded_win-producto_categoria', execute: 'renderAjaxNewWin'},\n";
    }
    else {
    };
  }
  ...
  {
    $producto_categoria = $object;
    return "{type: 'close_and_reedit', link: '".url_for($this->getUrlForAction('edit'), $object)."', link_content: '".url_for('producto_categoria/editWinContent?id='.$producto_categoria->getId())."'},\n";
  }
}