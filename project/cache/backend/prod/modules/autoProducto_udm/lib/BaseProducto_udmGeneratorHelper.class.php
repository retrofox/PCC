 ...
 */
class BaseProducto_udmGeneratorHelper extends sfModelGeneratorHelper
{
...
/*
  ...
  {
    return 'list' == $action ? 'producto_udm' : 'producto_udm_'.$action;
  }
*/

  public function getUrlForAction($action, $module = '')
  {
    $module_action = $module ? $module : 'producto_udm';
  ...
  public function mooJsonDataToEditObject($object, $params)
  {
    ...
    if (isset($params['ajax_request'])) {
      ...
      return "{type: 'ajax_link', link: '".url_for($this->getUrlForAction('edit'), $object)."', link_content: '".url_for('producto_udm/editWinContent?id='.$producto_udm->getId())."', update: '_new', execute: 'renderAjaxEditWin'},\n";
    }
    else {
  ...
  public function mooJsonDataToListObjects($object, $params)
  {
    ...
    if (isset($params['ajax_request'])) {
      ...
      return "{type: 'ajax_link', link: '".url_for('producto_udm')."', link_content: '".url_for('producto_udm/editWinContent?id='.$producto_udm->getId())."', update: '_new', execute: 'renderAjaxListWin'},\n";
    }
    else {
        return "{type: 'Guarda ACA !!'}";
    };
  ...
  public function mooJsonDataToNew($params, $cssClass='btn_admin_actions')
  {
    if (isset($params['inWinPopUp'])) {
      return "{type: 'ajax_link', link: '".url_for($this->getUrlForAction('new'))."', update: '_new', node_insert: 'embedded_win-producto_udm', execute: 'renderAjaxNewWin'},\n";
    }
    else {
    };
  }
  ...
  {
    $producto_udm = $object;
    return "{type: 'close_and_reedit', link: '".url_for($this->getUrlForAction('edit'), $object)."', link_content: '".url_for('producto_udm/editWinContent?id='.$producto_udm->getId())."'},\n";
  }
}