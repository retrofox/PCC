...
<ul class="sf_admin_bar_menu">
  <li class="btn110" id="btnFilterOn"><div class="icn icn-filter"></div><?php echo __('filter') ?></li>
    <?php echo moo_btn_to_filters(__('Reset', array(), 'sf_admin'), 'producto_udm_collection', array('action' => 'filter')) ?>
...
<div class="sf_admin_filter">
  <div id="sf_admin_filter_slide">
	  ...
	  <?php endif; ?>
    <form id ="sf_admin_filter_form" action="<?php echo url_for('producto_udm_collection', array('action' => 'filter')) ?>" method="post">
      <table class="sf_admin_filter_filters" cellspacing="0">
        <tbody>
          <tr>
	        <?php foreach ($configuration->getFormFilterFields($form) as $name => $field): ?>
	        <?php if ((isset($form[$name]) && $form[$name]->isHidden()) || (!isset($form[$name]) && $field->isReal())) continue ?>
	          <?php include_partial('producto_udm/filters_field', array(
	            'name'       => $name,
