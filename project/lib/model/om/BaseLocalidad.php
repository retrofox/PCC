...
/**
 * Base class that represents a row from the 'geo_localidad' table.
 *
 * 
 *
  ...
  const PEER = 'LocalidadPeer';
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			$this->provincia_id = ($row[$startcol + 1] !== null) ? (int) $row[$startcol + 1] : null;
			...
			return $startcol + 3; // 3 = LocalidadPeer::NUM_COLUMNS - LocalidadPeer::NUM_LAZY_LOAD_COLUMNS).
	...
	public function fromArray($arr, $keyType = BasePeer::TYPE_PHPNAME)
	{
		...
		if (array_key_exists($keys[1], $arr)) $this->setProvinciaId($arr[$keys[1]]);
	 ...
	 * related UserProfiles from storage. If this Localidad is new, it will return
		...
		if ($this->collUserProfiles === null) {
			if ($this->isNew()) {
			   $this->collUserProfiles = array();
				...
				$criteria->add(UserProfilePeer::LOCALIDAD_ID, $this->id);
	 ...
	 */
	public function getUserProfilesJoinsfGuardUser($criteria = null, $con = null, $join_behavior = Criteria::LEFT_JOIN)
	{
		if ($criteria === null) {
			$criteria = new Criteria(LocalidadPeer::DATABASE_NAME);
		}
		...
		if ($this->collUserProfiles === null) {
			if ($this->isNew()) {
				...
				$this->collUserProfiles = UserProfilePeer::doSelectJoinsfGuardUser($criteria, $con, $join_behavior);
			}
		} else {
			// the following code is to determine if a new query is
			// called for.  If the criteria is the same as the last
	 ...
	 * If this collection has already been initialized with an identical Criteria, it returns the collection.
	 * Otherwise if this Localidad has previously been saved, it will retrieve
	 * related Proveedors from storage. If this Localidad is new, it will return
	 * an empty collection or the current collection, the criteria is ignored on a new object.
	 *
	 * @param      PropelPDO $con
	 * @param      Criteria $criteria
		...
		if ($this->collProveedors === null) {
			if ($this->isNew()) {
			   $this->collProveedors = array();
				...
				$criteria->add(ProveedorPeer::LOCALIDAD_ID, $this->id);
	 ...
	 * actually need in Localidad.
	 */
	public function getProveedorsJoinProveedorRubro($criteria = null, $con = null, $join_behavior = Criteria::LEFT_JOIN)
	{
		if ($criteria === null) {
			$criteria = new Criteria(LocalidadPeer::DATABASE_NAME);
		}
		...
		if ($this->collProveedors === null) {
			if ($this->isNew()) {
				...
				$this->collProveedors = ProveedorPeer::doSelectJoinProveedorRubro($criteria, $con, $join_behavior);
			}
		} else {
			// the following code is to determine if a new query is
			// called for.  If the criteria is the same as the last
