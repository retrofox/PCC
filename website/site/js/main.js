...
window.addEvent ('load', function (){
  ...
  if (ctextos){
    ...
    var cantText = textos.length;

    //console.debug('cantidad textos',textos.length);
    var FxTextos = [];
    textos.each(function(texto, iT){
      var posL = iT/(cantText/2);
      posL=posL.toInt();
      var posT = 40*(iT-((cantText/2)*posL))+20;
     //console.debug('pos', posT);
      var tFx = FxTextos[iT] = new Fx.Morph(texto, {
          duration: 1000,
          transition: 'linear'
        });
